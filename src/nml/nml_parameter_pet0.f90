!> \file nml_parameter_pet0.f90
!> \copydoc nml_pet0

!> \brief PET - Case 0
!> \details Parameters for PET (case 0 - aspect correction).
!> \version 0.1
...
module nml_pet0
  ...
  contains
    procedure :: init => nml_pet0_init
    procedure :: from_file => nml_pet0_from_file
    procedure :: set => nml_pet0_set
    procedure :: is_set => nml_pet0_is_set
    procedure :: is_valid => nml_pet0_is_valid
  end type nml_pet0_t

contains

  !> \brief Initialize defaults and sentinels for pet0
  integer function nml_pet0_init(this, errmsg) result(status)
    class(nml_pet0_t), intent(inout) :: this
  ...
  end function nml_pet0_init

  !> \brief Read pet0 namelist from file
  integer function nml_pet0_from_file(this, file, errmsg) result(status)
    class(nml_pet0_t), intent(inout) :: this
    ...
    status = nml%find("pet0", errmsg=errmsg)
    ...
    ! read namelist
    read(nml%unit, nml=pet0, iostat=iostat, iomsg=iomsg)
    if (iostat /= 0) then
      status = NML_ERR_READ
    ...
    status = NML_OK
  end function nml_pet0_from_file

  !> \brief Set pet0 values
  integer function nml_pet0_set(this, &
    minCorrectionFactorPET, &
    maxCorrectionFactorPET, &
    aspectTresholdPET, &
    ...
    this%is_configured = .true.
    status = NML_OK
  end function nml_pet0_set

  !> \brief Check whether a namelist value was set
  integer function nml_pet0_is_set(this, name, idx, errmsg) result(status)
    class(nml_pet0_t), intent(in) :: this
  ...
  end function nml_pet0_is_set

  !> \brief Validate required values and constraints
  integer function nml_pet0_is_valid(this, errmsg) result(status)
    class(nml_pet0_t), intent(in) :: this
    character(len=*), intent(out), optional :: errmsg
    integer :: istat
  ...
  end function nml_pet0_is_valid
