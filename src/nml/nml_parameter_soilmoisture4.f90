!> \file nml_parameter_soilmoisture4.f90
!> \copydoc nml_soilmoisture4

!> \brief Soil moisture - Case 4
!> \details Feddes equation for ET reduction and global FC dependency on root fraction coefficient
!> \version 0.1
...
module nml_soilmoisture4
  ...
  contains
    procedure :: init => nml_soilmoisture4_init
    procedure :: from_file => nml_soilmoisture4_from_file
    procedure :: set => nml_soilmoisture4_set
    procedure :: is_set => nml_soilmoisture4_is_set
    procedure :: is_valid => nml_soilmoisture4_is_valid
  end type nml_soilmoisture4_t

contains

  !> \brief Initialize defaults and sentinels for soilmoisture4
  integer function nml_soilmoisture4_init(this, errmsg) result(status)
    class(nml_soilmoisture4_t), intent(inout) :: this
  ...
  end function nml_soilmoisture4_init

  !> \brief Read soilmoisture4 namelist from file
  integer function nml_soilmoisture4_from_file(this, file, errmsg) result(status)
    class(nml_soilmoisture4_t), intent(inout) :: this
    ...
    status = nml%find("soilmoisture4", errmsg=errmsg)
    ...
    ! read namelist
    read(nml%unit, nml=soilmoisture4, iostat=iostat, iomsg=iomsg)
    if (iostat /= 0) then
      status = NML_ERR_READ
    ...
    status = NML_OK
  end function nml_soilmoisture4_from_file

  !> \brief Set soilmoisture4 values
  integer function nml_soilmoisture4_set(this, &
    orgMatterContent_forest, &
    orgMatterContent_impervious, &
    orgMatterContent_pervious, &
    PTF_lower66_5_constant, &
    ...
    this%is_configured = .true.
    status = NML_OK
  end function nml_soilmoisture4_set

  !> \brief Check whether a namelist value was set
  integer function nml_soilmoisture4_is_set(this, name, idx, errmsg) result(status)
    class(nml_soilmoisture4_t), intent(in) :: this
  ...
  end function nml_soilmoisture4_is_set

  !> \brief Validate required values and constraints
  integer function nml_soilmoisture4_is_valid(this, errmsg) result(status)
    class(nml_soilmoisture4_t), intent(in) :: this
    character(len=*), intent(out), optional :: errmsg
    integer :: istat
  ...
  end function nml_soilmoisture4_is_valid

end module nml_soilmoisture4
