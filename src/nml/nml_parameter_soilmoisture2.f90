!> \file nml_parameter_soilmoisture2.f90
!> \copydoc nml_soilmoisture2

!> \brief Soil moisture - Case 2
!> \details Jarvis equation for ET reduction, multi-layer infiltration capacity approach, Brooks-Corey like
!> \version 0.1
...
module nml_soilmoisture2
  ...
  contains
    procedure :: init => nml_soilmoisture2_init
    procedure :: from_file => nml_soilmoisture2_from_file
    procedure :: set => nml_soilmoisture2_set
    procedure :: is_set => nml_soilmoisture2_is_set
    procedure :: is_valid => nml_soilmoisture2_is_valid
  end type nml_soilmoisture2_t

contains

  !> \brief Initialize defaults and sentinels for soilmoisture2
  integer function nml_soilmoisture2_init(this, errmsg) result(status)
    class(nml_soilmoisture2_t), intent(inout) :: this
  ...
  end function nml_soilmoisture2_init

  !> \brief Read soilmoisture2 namelist from file
  integer function nml_soilmoisture2_from_file(this, file, errmsg) result(status)
    class(nml_soilmoisture2_t), intent(inout) :: this
    ...
    status = nml%find("soilmoisture2", errmsg=errmsg)
    ...
    ! read namelist
    read(nml%unit, nml=soilmoisture2, iostat=iostat, iomsg=iomsg)
    if (iostat /= 0) then
      status = NML_ERR_READ
    ...
    status = NML_OK
  end function nml_soilmoisture2_from_file

  !> \brief Set soilmoisture2 values
  integer function nml_soilmoisture2_set(this, &
    orgmattercontent_forest, &
    orgmattercontent_impervious, &
    orgmattercontent_pervious, &
    ptf_lower66_5_constant, &
    ...
    this%is_configured = .true.
    status = NML_OK
  end function nml_soilmoisture2_set

  !> \brief Check whether a namelist value was set
  integer function nml_soilmoisture2_is_set(this, name, idx, errmsg) result(status)
    class(nml_soilmoisture2_t), intent(in) :: this
  ...
  end function nml_soilmoisture2_is_set

  !> \brief Validate required values and constraints
  integer function nml_soilmoisture2_is_valid(this, errmsg) result(status)
    class(nml_soilmoisture2_t), intent(in) :: this
    character(len=*), intent(out), optional :: errmsg
    integer :: istat
  ...
  end function nml_soilmoisture2_is_valid

end module nml_soilmoisture2
